...
void send_out_down_particles(vector<particle_t*> &out_down_particles,
  ...
  MPI_Send(out_down_particles.data(),
           out_down_particles.size() * sizeof(particle_t*), MPI_BYTE,
...
void send_out_up_particles(vector<particle_t*> &out_up_particles,
  ...
  MPI_Send(out_up_particles.data(),
           out_up_particles.size() * sizeof(particle_t*), MPI_BYTE,
           world_rank - 1, 0, MPI_COMM_WORLD);
...
void receive_above_particles(vector<particle_t*> &incoming_particles,
  ...
  MPI_Get_count(&status, MPI_BYTE, & incoming_particles_size);
  incoming_particles.resize(incoming_particles_size / sizeof(particle_t*));
...
void gather_for_save(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
  ...
  // std::cout << "step2";
  // int count = local_parts.size() * sizeof(particle_t);
  ...
  //  std::cout << "step4";
  //  MPI_Gatherv(local_parts.data(), local_parts.size()*sizeof(particle_t),MPI_BYTE ,
  //                parts, counts, 0, MPI_BYTE, 0, MPI_COMM_WORLD);

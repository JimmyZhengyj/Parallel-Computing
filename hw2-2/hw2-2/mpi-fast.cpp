...
std::map<int, std::vector<int>> send_grid_lens;
std::map<int, std::array<MPI_Request, 2>> sendreqs;
std::map<int, std::array<MPI_Request, 2>> recvreqs;

std::map<int, int> send_lens;
std::map<int, std::vector<particle_t>> send_parts;
...
void init_simulation(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    for(int i = 0; i < num_bins; ++i){
        ...
        for(int d = 0; d < 4; ++d){
            ...
            if(ni < 0 or ni >= dim or nj < 0 or nj >= dim)
                continue;
            int nei_rank = get_rank(ni * dim + nj);
    ...
    int farthest_bidx = MIN((last_bi + 1) * dim + (last_bj + 1), dim * dim-1);
    ...
    bin_dim = farthest_bidx - local_offset + 1;
    ...
    for(int i = 0; i < num_parts; ++i){
        int bidx = floor(parts[i].x / BINSIZE) * dim + floor(parts[i].y / BINSIZE);
...
void recv_axay(int src_rank,
    ...
    for(int i = 0; i < n;){
        if(grid_lens[idx] == 0){
            ...
            std::vector<particle_t> &grid = bins[bidx - local_offset];
...
void clear_grids_not_owned_by_me(){
    for(int i = num_bins; i < bin_dim; ++i){
        bins[i].clear();
    }
}
...
void simulate_one_step(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    for(auto &kv: rank_grids_send){
        ...
        std::vector<int> &grid_lens = send_grid_lens[target_rank];
        len = 0;
        parts_to_send.clear();
    ...
    // Clear [num_bins, bin_dim) in bins
    ...
    for(auto &kv: recvreqs){
        ...
        for(particle_t &p: parts_to_recv){
            int bidx = floor(p.x / BINSIZE) * dim + floor(p.y / BINSIZE);
    ...
    for(auto &kv: rank_grids_send){
        ...
        std::vector<int> &grid_lens = send_grid_lens[src_rank];
    ...
    for(int i = 0; i < num_bins; ++i){
        ...
        for(int j = grid_n-1; j >= 0; --j){
            ...
            move(p, size);
            //
            int new_bidx = floor(p.x / BINSIZE) * dim + floor(p.y / BINSIZE);
            if(new_bidx == local_offset + i){
                ...
                int target_rank = get_rank(new_bidx);
...
void gather_for_save(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    for(int i = 0; i < num_bins; ++i){
        ...
        num_parts_to_send += n;
        for(int j = 0; j < n; ++j){
            local_parts_send.push_back(bins[i][j]);
        }
    }

...
std::vector<int> send_sums, recv_sums;
...
void apply_force_inside(particle_t& particle, particle_t& neighbor) {
    // Calculate Distance
    double dx = neighbor.x - particle.x;
    double dy = neighbor.y - particle.y;
    double r2 = dx * dx + dy * dy;
...
void get_sums(vector<int> &num_particles, vector<int> &result){
...
void sets_request(unordered_map<int, set<int>> &temp_send_bins, 
                unordered_map<int, set<int>> &temp_receive_bins){
...
void init_simulation(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    bin_dim = MIN((right_bi + 1) * dim + (right_bj + 1), dim * dim-1) - subdomain_size + 1;
    bins.resize(bin_dim);
    ...
    // sets_request(temp_send_bins, temp_receive_bins);
    for(auto &it: temp_send_bins){
        int cur_rank = it.first;
        send_req[cur_rank];
        receive_req[cur_rank];
    ...
    send_sums.resize(num_procs);
    recv_sums.resize(num_procs);
}
...
void receive_particles(int rank, vector<particle_t> &receive_ps, array<MPI_Request, 2> &reqs){
...
void MPI_ALL(int num_procs, map<int, vector<particle_t>> &moving_out){
    ...
    get_sums(send_part_num, send_sums);
    get_sums(receive_part_num, recv_sums);

    // Send and receive the particles all to all
    MPI_Alltoallv(&send_part_all[0], &send_part_num[0], &send_sums[0], PARTICLE, 
                &receive_part_all[0], &receive_part_num[0], &recv_sums[0], PARTICLE, MPI_COMM_WORLD);
...
void simulate_one_step(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    for(auto &it: receive_bins){
        int cur_rank = it.first;
        vector<particle_t> &receive_ps = receive_parts[cur_rank];
        array<MPI_Request, 2> &reqs = receive_req[cur_rank];
        receive_particles(cur_rank, receive_ps, reqs);
    }
    ...
    for(int i = subdomain_start + num_bins; i < subdomain_size + bin_dim; ++i){
        bins[i - subdomain_size].clear();
    }
    ...
    for(int i = 0; i < num_bins; ++i){
        ...
        for(int j = 0; j < bin.size(); ++j){
            for(int k = j+1; k < bin.size(); ++k){
                apply_force_inside(bin[j], bin[k]);
            }
        }
    ...
    for(int i = 0; i < num_bins; ++i){
        ...
        for(int j = bin.size()-1; j >= 0; --j){
            ...
            if(new_bin_idx >= subdomain_start and new_bin_idx < subdomain_start + num_bins){
                not_leaving.push_back(p);
                bin.erase(bin.begin() + j);
            }else{
                int target_rank = get_rank(new_bin_idx);
...
void gather_for_save(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    if(rank == 0){
        ...
        MPI_Gatherv(send_part_all.data(), send_part_all.size(), PARTICLE, receive_part_all.data(),
                receive_part_num.data(), recv_sums.data(), PARTICLE, 0, MPI_COMM_WORLD);
